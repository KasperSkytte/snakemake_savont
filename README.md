...
| --- | --- | --- |
| `01-sample_prep.smk` | `sample_prep` | For each sample, decompress, concatenate all files into a single file, count reads before any filtering, relabel reads using `usearch -fastx_relabel`. |
| | `concatenate_total_reads_files` | Concatenates all files with number of reads per sample into a single file. |
| `02-savont.smk` | `concat_all` | Concatenates all filtered reads from all samples into a single file for ASV generation. |
...
| `04-abund_table.smk` | `abund_table` | For each sample, estimate ASV abundances in each sample by mapping the raw, unfiltered reads against the ASVs using `usearch -otutab`. This is MUCH faster to do in parallel and merge afterwards compared to running a single `usearch -otutab` command for all samples at once, because it doesn't scale linearly with more threads. |
| | `merge_abund_tables` | Merge all per-sample abundance tables into a single table using `usearch -otutab_merge`. |
...
Depending on the size of the data, you can use an executor if you are running on a HPC cluster to optimize utilization and significantly speed up the runtime by submitting individual tasks as separate jobs. See the `slurm_submit.sbatch` for an example when running on a SLURM cluster.
...
### Docker
The pre-built Docker container available from [`ghcr.io/kasperskytte/snakemake_savont:main`](https://github.com/KasperSkytte/snakemake_savont/pkgs/container/snakemake_savont) is built from the `Dockerfile` and includes all the required software (except snakemake itself, only the conda environment includes that).

### Conda
Requirements are listed in `environment.yml`. To create a conda environment simply run:
```
